...
namespace BetEuro.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                ModelState.AddModelError("", "Musisz potwierdzić e-mail.");
                return View(model);
            }            
            if (!user.isActive)
            {
                ModelState.AddModelError("", "Administrator nie aktywował jeszcze Twojego konta.");
                return View(model);
            }
            if (await SignInManager.PasswordSignInAsync(model.Username, model.Password, model.RememberMe, shouldLockout: false) == SignInStatus.Success)
            {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await UserManager.SendEmailAsync(user.Id, "Zmiana hasła.", "Proszę przejść do strony podanej poniżej w celu zmiany hasła " + Environment.NewLine + callbackUrl);
